...
import { categoryAPI, incomeAPI } from "../services/api"; // For fetching categories
...
function Income() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter categories to only show 'income' type for the dropdown
  ...
  // Helper function to find category details by ID for display purposes
  ...
  const handleOpenModal = () => {
    ...
    // Pre-select the first category in the dropdown if it exists
      ...
      <main className="flex-1 w-full flex flex-col items-center py-12 px-4">
        <div className="w-full max-w-5xl bg-muted p-6 md:p-8 rounded-lg border border-slate-800">
          ...
          <div className="mt-6 border-t border-slate-700">
            {/* Table Header (This is now part of the stable skeleton) */}
            ...
            {/* Table Body (Conditionally rendered content) */}
            {loading ? (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 justify-end">
                      <button
                        ...
                        aria-label={`Delete ${income.incomeSource}`}
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
